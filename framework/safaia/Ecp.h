...
namespace Safaia{
    ...
    public:
        static std::string render(std::string ecp_path, EcpAtom atom){
            std::string view;
            if (enable_ecp_cache){
                ...
                if (cache_got == ecp_cache.end()){
                    ...
                    // Add to cache
                    ecp_cache.insert(std::pair<std::string,std::string>(ecp_path, view));
                } else {
                    // Cache Found
            ...
            for(auto it = atom.dic.begin(); it != atom.dic.end(); it++){
                replace_all(view, "<=$" + it -> first + "=>", it -> second);
            }
